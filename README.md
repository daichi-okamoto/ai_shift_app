...
- 表示中の期間に対するメンバー別のシフト種別回数と、公平ポイント（夜勤／週末／祝日内訳付き）を可視化。
- シフト表を全画面モードに拡張し、集中して編集できる表示切替。
...
| ロール | 権限 |
|--------|------|
| 管理者 | 全ユーザー・全シフトの閲覧／編集、事業所／ユニット作成、監査ログ管理、勤務希望の代理登録（全メンバー対象） |
| チームリーダー | 担当ユニットのシフト作成、メンバー管理、AI 編成実行、公開承認、勤務希望の代理登録（担当ユニット内メンバー） |
| 一般ユーザー | 自ユニットのシフト閲覧、希望提出、休暇申請（本人のみ） |
...
#### 4.3 希望シフト / 休暇
- 提出依頼の自動通知
- 管理者は事業所内の任意メンバーに対して代理申請が可能
- チームリーダーは担当ユニットのメンバーに対して代理申請が可能
- 締切後も変更可（代理登録含む）
- 休暇申請は承認不要で自動反映
...
- `availability_reminder_tasks`：希望リマインドの自動送信予定と履歴
